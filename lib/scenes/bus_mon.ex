defmodule ModsynthGui.Scene.BusMon do
  ...
  def handle_info(:update, %State{graph: graph, ets_state: ets_state} = state) do
    graph =
  ...
  def do_mon_if_already_loaded(_graph, _nodes, connections) when is_nil(connections) do
    nil
  end

  def do_mon_if_already_loaded(graph, nodes, connections) do
    {columns, rows} = monitor_data(nodes, connections)
    Scenic.Table.Components.table(graph, columns, rows, t: {200, 50}, color: :light_grey, id: 999)
  end

  def monitor_data(_nodes, connections) do
